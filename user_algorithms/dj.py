...
def goodSells(symbList, verbose=False): #symbList is a list of stocks ready to be sold
  lock = o.threading.Lock()
  lock.acquire()
  posList = o.json.loads(open(c['file_locations']['posList'],'r').read())['algos'][algo] #load up the stock data for the algo
  lock.release()
...
def goodSell(symb):
  ...
  lock = o.threading.Lock()
  lock.acquire()
  stockList = o.json.loads(open(c['file_locations']['posList'],'r').read())['algos'][algo]
  lock.release()
