...
from extendBrokers.utilities import bcolor
import pprint
# from extendBrokers.utilities import account_performance
...
import extendBrokers.extend_alpacafxns  as multistonk_alpacafxns
try:
...
configFile = toml.load(Path().absolute()/'robinhood_config.toml')
algoList = configFile['allAlgos']['algoList']
algoList = {e:{} for e in algoList}
print(f"Key file \t{configFile['file_locations']['keyFile']}")
print(f"posList file\t{configFile['file_locations']['posList']}")
print(f"buyList file\t{configFile['file_locations']['buyList']}")
print(f"Error log \t{configFile['file_locations']['errLog']}")
print("Using the algos: ",end="")
print(*list(algoList),sep=", ",end="\n\n")


multistonk_alpacafxns.init(keyFile = configFile , isPaper = int(configFile['account_params']['isPaper']))

sys.path.append(configFile['file_locations']['user_stockAlgosDir'])
...
minCashMargin = float(configFile['account_params']['minCashMargin']) #extra cash to hold above hold value
...
minCash2hold = float(configFile['account_params']['minCash2hold'])
maxCash2hold = float(configFile['account_params']['maxCash2hold'])
...
def main(verbose=False):
	...
	multistonk_alpacafxns.checkValidKeys(int(configFile['account_params']['isPaper']))
	if (len(account_details.getPosition()) == 0):
		print(f"Will start buying {configFile['time_params']['buyTime']} minutes before next close")
	else:
		pass
	...
	isManualSellOff = not int(configFile['account_params']['portAutoSellOff'])
	while True:
		# acct = multistonk_alpacafxns.getAcct() #get account info
		pos = multistonk_alpacafxns.getPos() #get all held positions (no algo assigned)
		# pprint.pprint(pos)
		robinhood_account = account_details.getRobinhood_Acct()
		accont_pos = account_details.getPosition()

		if(ask2sell and float(robinhood_account['equity'])<float(maxPortVal)*float(configFile['account_params']['portStopLoss']) and len(accont_pos)>0):
		...
		if(multistonk_alpacafxns.marketIsOpen()):
			...
			print("----\t------\t-----\t-----------\t-----------\t-----------\t----------")
			#look to sell things
			holdings = account_details.unrealized_()
			# holdings = account_details.get_portfolio_symbols()
			check2sells(pos = holdings)
			if((closeTime-dt.datetime.now()).total_seconds()<=60*float(configFile['time_params']['buyTime']) and sum([t.getName().startswith('update') for t in o.threading.enumerate()])==0):
			...
			print("Algo ROI estimated:"+"\n")
			for algo in algoList:
				curPrices = o.getPrices([e+"|stocks" for e in posList[algo]]) #get the current prices of all the stocks in a given algo
				algoCurVal = sum([posList[algo][s]['sharesHeld']*curPrices[s+"|stocks".upper()]['price'] for s in posList[algo] if s+"|stocks".upper() in curPrices])+cashList[algo]['earned']
			...
			print("\n")
			# portHist = account_details.getRobinbood_ProfileHistory() #get the closing prices of the portfolio over the last month
			# print(portHist)
			Robinhood_portfolio_history = account_details.getRobinbood_ProfileHistory()
			float_maxPort = max([e for e in Robinhood_portfolio_history if e is not None]) #get the max portfolio value over the last month and remove blank entries
			maxPortVal = float(float_maxPort)
			# print(maxPortVal)
			print(f"Portfolio history: {len(float_maxPort)} days")
			print(f"\nHighest portVal in the last month: ${round(maxPortVal,2)}")
			syncPosList()
			if(o.dt.date.today().weekday()==4 and o.dt.datetime.now().time()>o.dt.time(12)):
				print("Removing saved csv files")
				for f in glob(configFile['file_locations']['stockDataDir']+"*.csv"):	
			...
			tto = multistonk_alpacafxns.timeTillOpen()
			...
			time.sleep(multistonk_alpacafxns.timeTillOpen())
...
def updateLists(verbose=False):
	...
	if(o.os.path.isfile(configFile['file_locations']['buyList'])):
		if(verbose):
			print("File is present. Checking mod date")
		try:
			modDate = dt.datetime.strptime(time.strftime("%Y-%m-%d",time.localtime(os.stat(configFile['file_locations']['buyList']).st_mtime)),"%Y-%m-%d").date() #if ANYONE knows of a better way to get the modified date into a date format, for the love of god please let me know
		except Exception:
			modDate = dt.date.today()-dt.timedelta(1)
		if(modDate==dt.date.today()):
			try:
				if(verbose): print("Reading from file")
				f = json.loads(open(configFile['file_locations']['buyList'],'r').read())
	...
	if(errored):
		...
		if(not exitFlag):
			...
			with open(configFile['file_locations']['buyList'],'w') as f:
...
def check2buy(algo, cashAvailable, stocks2buy, verbose=False):
	...
	if(len(stocks2buy)>0):
		cashPerStock = min(cashAvailable,max(float(configFile['account_params']['minDolPerStock']),cashAvailable/len(stocks2buy)))
	...
	for stock in stocks2buy:
		...
		if(not exitFlag): #ensure that the exitFlag is not set
			...
			if lastTradeDate < dt.date.today() and stockInfo['lastTradeType']!="sell":
				...
				if(curPrice>0):
					shares = int(min(cashPerStock/curPrice,(mktCap/curPrice)*float(configFile['account_params']['maxVolPerc'])))
				else:
					shares = 0
				...
				if(verbose):
					...
					print(f"{algo} - {stock} - {curPrice} - ok to buy {shares} shares")
...
def triggeredUp(symb, algo):
	...
	if(isSold):
		...
		print(f"Sold all shares of {symb} for {algo} algo")
		...
		for e in algoSymbs: #go through the stocks of the algo
			if(posList[algo][e['symbol']]['lastTradeDate']<str(dt.date.today()) or posList[algo][e['symbol']]['lastTradeType']!='buy'): #only display/sell if not bought today
				...
				if(gs[e['symbol']]==1): #if the stock is a good sell (sellUp)
					...
					#else if it sells down (stop-loss)
					# print("#sell immediately")
					print(e['symbol'])
					sell(e['symbol'],algo)
...
def sell(stock, algo):
	...
	if(posList[algo][stock]['sharesHeld']>0):
		r = account_details.createOrder(side="sell",symbol=stock,shares= posList[algo][stock]['sharesHeld'])
		# account_details.createOrder(stock, 'market', 'sell', posList[algo][stock]['sharesHeld'])
		# r = {
		# 	"status": "accepted",
		# }
	...
	if('state' in r and r['state'] == "unconfirmed"): #check that it actually sold
		...
		open(configFile['file_locations']['posList'],'w').write(json.dumps({'algos':posList,'cash':cashList},indent=2)) #update the posList file
		triggeredStocks.discard(algo+"|"+stock)
		lock.release()
...
def buy(shares, stock, algo, buyPrice):	
	print(f"{bcolor.OKGREEN}Buying {shares} shares of {stock} with {algo} at {buyPrice}{bcolor.ENDC}")
	r = account_details.createOrder(side = "buy",symbol = stock,shares= shares) #this needs to happen first so that it can be as accurate as possible
	# r = {
	# 	"status" : "accepted"
	# }
	...
	if('state' in r and r['state'] == "unconfirmed"):
		...
		lock.acquire()
		posList[algo][stock] = { #update the entry in posList
			"sharesHeld":float(posList[algo][stock]['sharesHeld'])+float(r['quantity']) if stock in posList[algo] else float(r['quantity']),
			"lastTradeDate":str(dt.date.today()),
			"lastTradeType":"buy",
			#running avg = (prevAvg*n+newAvg*m)/(n+m)
			"buyPrice":(posList[algo][stock]['buyPrice']*posList[algo][stock]['sharesHeld']+buyPrice*float(r['quantity']))/(posList[algo][stock]['sharesHeld']+float(r['quantity'])) if stock in posList[algo] else buyPrice,
			"shouldSell":False,
			"note":algoList[algo][stock] if stock in algoList[algo] else ""
		}
		
		cashList[algo]['invested'] += buyPrice*shares
		open(configFile['file_locations']['posList'],'w').write(json.dumps({'algos':posList,'cash':cashList},indent=2)) #update posList file
		lock.release()
		return True
...
def setPosList(algoList, verbose=True):
	#this function sets the position list to be used by the program
	posList={}
	cashList = {}
	#if the posList file doesn't exist
	if(not os.path.isfile(configFile['file_locations']['posList'])):

		if(verbose): print("File is missing. Creating and adding blank lists...")
		lock = o.threading.Lock()
		lock.acquire()
		with open(configFile['file_locations']['posList'],'w') as f:
		...
		lists = json.loads(open(configFile['file_locations']['posList'],'r').read())
		...
		lock = o.threading.Lock()
		lock.acquire()
		with open(configFile['file_locations']['posList'],'r') as f:
...
def syncPosList(verbose=False):
	...
	# p = a.getPos()
	holdings = account_details.get_portfolio_symbols()
	heldPos = {e['symbol']:float(e['quantity']) for e in holdings} #actually held positions
	heldBuyPrices = {e['symbol']:float(e['average_buy_price']) for e in holdings} #get the actual buy prices for each stock
	# print(heldPos,heldBuyPrices)
	...
	if(not eq(recPos, heldPos) or not os.path.isfile(configFile['file_locations']['buyList'])): #compare again after the initial comparison
		if(verbose): 
			print(f"Discrepency still present or buyList file is missing. Updating buy list")
...
if __name__ == '__main__':	
	...
	try:
		...
		traceback.print_exc(file=open(configFile['file_locations']['errLog'],"a"))
